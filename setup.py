...
class Config(object):
    ...
    def compile_args(self, mod):
        ...
        if platform.system() == "Windows":
            ...
            if bits:
                ...
                # Only add curl include if library files exist
                curl_lib_path = os.path.join(bits, "curl", "libraries")
                if os.path.exists(curl_lib_path) and os.listdir(curl_lib_path):
                    args.append(quote_path("-I" + os.path.join(bits, "curl", "include")))
            else:
                args = []
        else:
            args = [
    ...
    def libraries(self, mod):
        ...
        if platform.system() == "Windows":
            if mod == "_tl":
                return ["expat", "pthreadVCE2", "zlib", "wsock32", "libpng16", "libcurl"]
        ...
        if platform.system() == "Windows":
            ...
            if bits:
                args += [
                    ...
                    quote_path("/LIBPATH:" + os.path.join(bits, "ptw", "lib")),
                    quote_path("/LIBPATH:" + os.path.join(bits, "png", "libraries")),
                    quote_path("/LIBPATH:" + os.path.join(bits, "expat", "lib")),
                ]
                # Only add curl library path if library files exist
                curl_lib_path = os.path.join(bits, "curl", "libraries")
                if os.path.exists(curl_lib_path) and os.listdir(curl_lib_path):
                    args.append(quote_path("/LIBPATH:" + curl_lib_path))
            return args
        elif platform.system() == "Darwin":
    ...
    def macros(self):
        ...
        macros = [
            ...
            ("GSI_ALIAS_INSPECT", 1),
            ("XML_STATIC", 1),
            # Map expat function names to the underscore-prefixed versions for Windows static linking
        ...
        # Only add HAVE_CURL if curl library is available on Windows
        if platform.system() == "Windows":
            bits = os.getenv("KLAYOUT_BITS")
            if bits:
                curl_lib_path = os.path.join(bits, "curl", "libraries")
                if os.path.exists(curl_lib_path) and os.listdir(curl_lib_path):
                    macros.append(("HAVE_CURL", 1))
            # For pthread static library linking on Windows
            macros.append(("PTW32_STATIC_LIB", 1))
        else:
            # On non-Windows platforms, assume curl is available
            macros.append(("HAVE_CURL", 1))
...
# Exclude pthread-dependent sources on Windows (tlThreads.cc requires pthread.h)
if platform.system() == "Windows":
    # _tl_sources.discard(os.path.join(_tl_path, "tlThreads.cc"))  # Keep this - we have pthread
    _tl_sources.discard(os.path.join(_tl_path, "tlWebDAV.cc"))
